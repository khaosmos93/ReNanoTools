...
config = config()
config.General.workArea        = 'crab_nanoProd2017_'+date
config.General.transferOutputs = True
config.General.transferLogs    = False
config.JobType.sendExternalFolder = True
config.JobType.pluginName  = 'Analysis'
config.JobType.psetName    = 'NanoAODv9_2017_cfg.py'
...
if __name__ == '__main__':
    ...
    for data in datasets:
        ...
        if "USER" in data:
            ...
            config.Data.outputDatasetTag = 'RunIISummer20UL17MiniAODv2-NANOAOD'
        elif "mc2017" in data:
            dirName = data.split('/')[1]
            print (dirName)
            config.General.requestName = dirName
